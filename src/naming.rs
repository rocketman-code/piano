...
///
/// Uses `quote!` for token rendering, then normalizes whitespace so that
/// the output matches how source code writes types (e.g. `W<u32>` not
/// `W < u32 >`). This ensures FnCollector (syn-based) and InjectionCollector
/// (CST-based) produce matching type strings.
pub fn render_type(ty: &syn::Type) -> String {
    let raw = match ty {
        ...
        syn::Type::Group(t) => {
            ...
            return render_type(&t.elem);
        }
        syn::Type::ImplTrait(t) => quote!(#t).to_string(),
        syn::Type::Infer(_) => return "_".to_string(),
        syn::Type::Macro(t) => quote!(#t).to_string(),
        syn::Type::Never(_) => return "!".to_string(),
    ...
    normalize_type_str(&raw)
}

/// Normalize whitespace in a type string so that `quote!` output matches
/// source-code style: `W<u32>` instead of `W < u32 >`, `& Foo` -> `&Foo`, etc.
fn normalize_type_str(s: &str) -> String {
    ...
    while let Some(c) = chars.next() {
        match c {
            ' ' => {
                // Drop spaces adjacent to punctuation: < > , & * [ ] ( ) ;
                let prev = result.chars().last();
                let next = chars.peek().copied();
                let prev_is_punct = prev
                    .map(|p| "<>,&*[]();!".contains(p))
                    .unwrap_or(true);
                let next_is_punct = next
                    .map(|n| "<>,&*[]();!".contains(n))
                    .unwrap_or(true);
...
mod tests {
    ...
    fn run_pipeline(source: &str, module_prefix: &str) -> (Vec<String>, String) {
        ...
        // Build measured map: minimal_name -> u32 id (same as main.rs).
        let mut measured: std::collections::HashMap<String, u32> = std::collections::HashMap::new();
        for (i, qf) in functions.iter().enumerate() {
            measured.insert(qf.minimal.clone(), i as u32);
        }

        // Build all_instrumentable set (bare function names).
        let all_instrumentable: std::collections::HashSet<String> =
            functions.iter().map(|qf| qf.minimal.clone()).collect();

        // Build macro_name_ids (empty for these tests).
        let macro_name_ids: std::collections::HashMap<String, u32> =
            std::collections::HashMap::new();

        let result = crate::rewrite::instrument_source(
            source,
            &measured,
            &all_instrumentable,
            false,
    ...
    fn cross_path_agreement_basic() {
        ...
        for (source, prefix, desc) in &cases {
            ...
            // Verify that InjectionCollector found every function that FnCollector
            // discovered, by checking that each function got a guard injected.
            // The guard uses __piano_ctx.enter(name_id) with numeric IDs.
            for (i, _name) in display_names.iter().enumerate() {
                let guard = format!("__piano_ctx.enter({})", i);
                assert!(
                    instrumented.contains(&guard),
                    "{desc}: guard not found for id {i} ('{_name}') in:\n{instrumented}"
                );
            }
        }
    }
    ...
    fn cross_path_agreement_trait_impl() {
        ...
        let guard = format!("__piano_ctx.enter({})", 0);
        assert!(
            instrumented.contains(&guard),
            "guard not found in:\n{instrumented}"
        );
