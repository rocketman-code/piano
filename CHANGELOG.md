...
inside `macro_rules!` now just work.

### Added

- `--uncorrected` flag for `piano report`: show measured times without subtracting profiling overhead, for users who want to see the raw numbers
- `macro_rules!` functions with metavariable names (`fn $name()`) now appear in profiles, not just literal names (#452, #465)
- CPU measurement overhead (`cpu_bias_ns`) reported in NDJSON header alongside wall-time overhead, so both clocks are transparent
- Bias correction applied by default: reported self-time subtracts the calibrated per-call measurement overhead (aggregate correction, not per-sample, to avoid clipping)
- `total_ms` populated from `inclusive_ns` in aggregate NDJSON format, giving reports access to both self-time and inclusive time
...
- NDJSON header includes `run_id` and `timestamp_ms` fields (restored after rebuild)

### Fixed

- `piano tag` failed with "no piano runs found" on new-format NDJSON files because the header was missing `run_id`
- Functions inside `macro_rules!` invoked within `impl` blocks now get qualified names (`S::method` instead of `method`)
- Async functions generated by `macro_rules!` expansion now get `enter_async` wrapping instead of sync-only guards
