...
mod tests {
    ...
    #[test]
    fn pack_name_depth_or_not_xor() {
        // If | is replaced with ^, packing would zero out bits that overlap.
        // Since name_id is shifted left 16, and depth occupies the low 16,
        // there is no overlap so | and ^ happen to be equivalent for
        // non-overlapping inputs. We need overlapping bits to distinguish:
        // pack_name_depth packs into [unused:32][name_id:16][depth:16].
        // With non-overlapping fields, | and ^ produce the same result,
        // so this mutation is semantically equivalent and unkillable.
        // But let's at least verify round-trip correctness:
        let packed = pack_name_depth(0x1234, 0x0056);
        assert_eq!(unpack_name_id(packed), 0x1234);
        assert_eq!(unpack_depth(packed), 0x0056);

        // Edge case: both fields at max 16-bit value
        let packed_max = pack_name_depth(0xFFFF, 0xFFFF);
        assert_eq!(unpack_name_id(packed_max), 0xFFFF);
        assert_eq!(unpack_depth(packed_max), 0xFFFF);
    }

    #[test]
    fn drop_cold_frame_boundary_logic() {
        // Tests the frame boundary detection in drop_cold (lines 968-969).
        // The logic is:
        //   remaining_all_base = s.iter().all(|e| unpack_depth(e.packed) == 0)
        //   is_frame_boundary = unpack_depth(entry.packed) == 0 || remaining_all_base
        //
        // We verify this by using enter/register/drop at various depths and
        // checking that records are correctly flushed.
        reset();

        // Depth-0 function should flush on drop (frame boundary).
        {
            let _g = enter("frame_boundary_fn");
            register("frame_boundary_fn");
        }
        // After dropping, records should be flushed to THREAD_RECORDS.
        let records = collect_all();
        assert!(
            records.iter().any(|r| r.name == "frame_boundary_fn"),
            "depth-0 function should produce a record after drop"
        );
    }

    #[test]
    fn aggregate_division_not_modulo() {
        // Line 1087: cpu_self_ns as f64 / 1_000_000.0
        // If / is replaced with %, the result would be 0.0 for values < 1M
        // and wrong for values >= 1M.
        reset();
        {
            let _g = enter("agg_div_test");
            register("agg_div_test");
            // Burn some CPU to ensure non-zero elapsed time.
            let mut x = 0u64;
        ...
        let records = collect_all();
        let rec = records.iter().find(|r| r.name == "agg_div_test").unwrap();
        // total_ms should be > 0 and finite (not NaN/Inf).
        assert!(
            rec.total_ms > 0.0 && rec.total_ms.is_finite(),
            "total_ms should be positive and finite, got {}",
            rec.total_ms
        );
    }
}
