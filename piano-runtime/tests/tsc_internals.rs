...
#[test]
#[serial]
fn store_ratio_produces_correct_qm_values() {
    let _g = RatioGuard::new();

    // 125/3: Q=41, R=2, M=ceil(2 * 2^64 / 3) = 12297829382473034411
    tsc::store_ratio(125, 3);
    assert_eq!(tsc::load_quotient(), 41, "Q must be 125 / 3 = 41");
    assert_eq!(
        tsc::load_multiplier(),
        12_297_829_382_473_034_411,
        "M must be ceil(2 * 2^64 / 3)"
    );
}

#[test]
#[serial]
fn store_ratio_integer_ratio_has_zero_multiplier() {
    let _g = RatioGuard::new();

    // 4/2 = exact integer 2, remainder 0 -> M must be 0
    tsc::store_ratio(4, 2);
    assert_eq!(tsc::load_quotient(), 2, "Q must be 4 / 2 = 2");
    assert_eq!(
        tsc::load_multiplier(),
        0,
        "M must be 0 for exact integer ratio"
    );
}

#[test]
#[serial]
fn store_ratio_exact_power_of_two_divisor() {
    let _g = RatioGuard::new();

    // 3/2: Q=1, R=1, product = 1*2^64, product % 2 = 0 (exact division).
    // M = 2^64 / 2 = 2^63. This kills mutants that corrupt the ceil
    // rounding branch (% -> / or +, > -> >=) because those only diverge
    // when the u128 remainder is exactly zero.
    tsc::store_ratio(3, 2);
    assert_eq!(tsc::load_quotient(), 1, "Q must be 3 / 2 = 1");
    assert_eq!(
        tsc::load_multiplier(),
        9_223_372_036_854_775_808, // 2^63
        "M must be exactly 2^63 (no ceil rounding needed)"
    );
}

#[test]
#[serial]
fn store_ratio_remainder_differs_from_quotient() {
    let _g = RatioGuard::new();

    // 7/3: Q=2, R=1. Kills `% -> /` in store_ratio because
    // numer / denom = 2, but numer % denom = 1, and (2 mod 3) != (1 mod 3),
    // so compute_multiplier would receive the wrong remainder.
    // M = ceil(1 * 2^64 / 3) = 6148914691236517206
    //
    // Pre-set M to a sentinel so the assertion fails if store_multiplier
    // is mutated to a no-op.
    tsc::store_multiplier(0);
    tsc::store_ratio(7, 3);
    assert_eq!(tsc::load_quotient(), 2, "Q must be 7 / 3 = 2");
    assert_eq!(
        tsc::load_multiplier(),
        6_148_914_691_236_517_206,
        "M must be ceil(1 * 2^64 / 3)"
    );
}
