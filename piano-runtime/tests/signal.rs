...
// Piano's signal handler restores the previous handler.
// Install a custom handler, then init a session (which registers piano's
// handler). Raise SIGINT. Piano's handler should restore our handler
// and re-raise. Our handler should run.
#[test]
fn signal_handler_restores_previous() {
    ...
    // 2. Writes trailer
    // 3. Restores user_handler via signal(SIGINT, prev)
    // 4. Re-raises SIGINT via raise(sig)
    // Then user_handler runs, sets USER_HANDLER_RAN = true.
